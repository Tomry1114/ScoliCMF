...
def main():
    ...
    ap.add_argument("--attn",default="vanilla",choices=["vanilla","ttt"])
    a=ap.parse_args(); dev="cuda" if torch.cuda.is_available() else "cpu"; torch.manual_seed(a.seed); np.random.seed(a.seed)
    H,W=480,240
    model=MFDiT(img_size=(H,W),patch_size=8,data_channels=1,cond_channels=1,dim=384,depth=12,num_heads=6,text=(a.text!="off"),attn_type=a.attn).to(dev)
    mf=MeanFlow(channels=1,image_size=H,normalizer=['mean_std',[0.5],[0.5]],flow_ratio=0.75,time_dist=['lognorm',-0.4,1.0])
    print("CMF attn=%s text=%s trainable=%.2fM"%(a.attn,a.text,sum(p.numel() for p in model.parameters())/1e6),flush=True)
