...
for stem in ["scm_secant","shmm_v2"]:
    ...
    with torch.no_grad():
        ...
        bn=torch.nn.functional.normalize(d["m_static"]*0+torch.nn.functional.normalize(raw,dim=-1),dim=-1)
    # recompute tok_cos/E_top4 from a fresh forward aux
    cond.diag=False
    _,aux=cond(xp,r,t,torch.rand(8,cfg["model"]["dim"],device=dev),torch.rand(8,cfg["model"]["dim"],device=dev))
    print("%-11s tok_cos=%.4f E_top4=%.4f | R_removed_amp=%.4f R_removed_energy=%.4f"%(
        stem,float(aux["tok_cos"]),float(aux["E_top4"]),e_amp,e_eng))
print("TOK_DIAG_DONE")
